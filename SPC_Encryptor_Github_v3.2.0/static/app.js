...
async function createNewSession() {
    try {
        ...
        if (result.success) {
            ...
            // === THÊM: Cập nhật seed hiển thị nếu có ===
            const seedInput = document.getElementById('seedInput');
            if (seedInput && seedInput.value) {
                updateSeedDisplay(seedInput.value);
            }
        } else {
...
function toggleSessionMode() {
    ...
    const badge = document.getElementById('seedModeBadge');
    const hint = document.getElementById('seedDisplayHint');

    // Kiểm tra element tồn tại
    if (!btn || !badge || !hint) {
        console.error('Missing elements for toggleSessionMode');
        return;
    }

    if (sessionMode) {
        btn.textContent = '🔒 Kèm phiên (BẬT)';
        btn.style.background = '#00ff88';
        btn.style.color = '#0a0a1a';
        btn.style.border = 'none';
        badge.textContent = '🔒 Kèm phiên';
        badge.style.color = '#00ff88';
        badge.style.background = 'rgba(0,255,136,0.2)';
        hint.textContent = '💡 Seed này đã gắn với phiên trình duyệt hiện tại';
        showStatus('✅ Đã bật chế độ Kèm phiên', 'success');
    } else {
        btn.textContent = '🔓 Không kèm phiên (TẮT)';
        btn.style.background = 'transparent';
        btn.style.border = '1px solid #ff6b6b';
        btn.style.color = '#ff6b6b';
        badge.textContent = '🔓 Không kèm phiên';
        badge.style.color = '#ff6b6b';
        badge.style.background = 'rgba(255,107,107,0.2)';
        hint.textContent = '⚠️ Seed thuần, KHÔNG gắn với phiên (dùng cho file cũ)';
        showStatus('⚠️ Đã tắt chế độ Kèm phiên', 'warning');
    }

    // Cập nhật lại seed hiển thị nếu có
    const seedInput = document.getElementById('seedInput');
    if (seedInput && seedInput.value) {
        updateSeedDisplay(seedInput.value);
    }
...
function updateSeedDisplay(seed) {
    ...
    if (!display || !hint) {
        console.error('Missing seed display elements');
        return;
    }

    if (!seed) {
        display.textContent = 'Chưa có seed';
        return;
    }

    if (sessionMode && browserSessionId) {
        display.textContent = seed + '_' + browserSessionId;
        display.style.color = '#00ff88';
        hint.textContent = '💡 Seed này đã gắn với phiên trình duyệt hiện tại';
    } else {
        display.textContent = seed;
        display.style.color = '#f39c12';
        hint.textContent = '⚠️ Seed thuần, KHÔNG gắn với phiên';
    }
}
...
function generateSeed() {
    ...
    updateSeedDisplay(seed);  // === SỬA: Gọi hàm mới ===
...
async function encrypt() {
    ...
    if (!text) {
        showStatus('⚠️ Vui lòng nhập dữ liệu!', 'error');
        return;
    }
    ...
    console.log('🔍 [ENCRYPT] sessionMode:', sessionMode);
    console.log('🔍 [ENCRYPT] browserSessionId:', browserSessionId);
    console.log('🔍 [ENCRYPT] seed input:', seed);
    ...
    try {
        ...
        const result = await response.json();
        console.log('🔍 [ENCRYPT] Result from server:', result);
        ...
        const seedHint = document.getElementById('seedDisplayHint');
        const keyPhienDisplay = document.getElementById('keyPhienDisplay');
        const seedInput = document.getElementById('seedInput');

        console.log('🔍 [ENCRYPT] result.seed:', result.seed);
        console.log('🔍 [ENCRYPT] result.seed_with_session:', result.seed_with_session);

        if (seedDisplay) {
            let displaySeed = result.seed;

            // Nếu seed rỗng, thử lấy từ các field khác
            if (!displaySeed || displaySeed === '') {
                if (result.seed_with_session && result.seed_with_session !== '') {
                    displaySeed = result.seed_with_session;
                } else if (result.effective_seed && result.effective_seed !== '') {
                    displaySeed = result.effective_seed;
                } else if (sessionMode && browserSessionId) {
                    displaySeed = browserSessionId;
                } else {
                    displaySeed = 'ERROR_NO_SEED';
                }
            }

            console.log('🔍 [ENCRYPT] displaySeed:', displaySeed);

            seedDisplay.textContent = displaySeed;

            if (sessionMode && browserSessionId) {
                seedDisplay.style.color = '#00ff88';
                if (seedHint) seedHint.textContent = '💡 Seed này đã gắn với phiên trình duyệt hiện tại';
            } else {
                seedDisplay.style.color = '#f39c12';
                if (seedHint) seedHint.textContent = '⚠️ Seed thuần, KHÔNG gắn với phiên';
            }

            if (keyPhienDisplay) {
                keyPhienDisplay.textContent = displaySeed;
                keyPhienDisplay.style.color = '#f39c12';
            }
        } else {
            console.error('❌ [ENCRYPT] seedDisplay not found!');
        }

        // Tự động điền seed vào input
        if (seedInput) {
            if (sessionMode && browserSessionId) {
                seedInput.value = result.seed + '_' + browserSessionId;
            } else {
...
async function decrypt() {
    ...
    console.log('🔍 [DECRYPT] seed input:', seed);
    console.log('🔍 [DECRYPT] order:', order);
    console.log('🔍 [DECRYPT] browserSessionId:', browserSessionId);

    if (!input) {
        showStatus('⚠️ Vui lòng nhập Base64!', 'error');
        return;
    }
    if (!seed) {
        showStatus('⚠️ Vui lòng nhập seed!', 'error');
        return;
    }
    if (!saoyut) {
        showStatus('⚠️ Vui lòng nhập SAOYUT!', 'error');
        return;
    }

    // === KIỂM TRA SAOYUT ===
    if (!saoyut.techniques || saoyut.techniques.length === 0) {
        showStatus('⚠️ SAOYUT không có techniques!', 'error');
        return;
    }

    console.log('🔍 [DECRYPT] SAOYUT techniques:', saoyut.techniques.map(t => t.name));
    console.log('🔍 [DECRYPT] SAOYUT seed_hash:', saoyut.seed_hash);
    console.log('🔍 [DECRYPT] SAOYUT browser_session_id:', saoyut.browser_session_id);

    // === KIỂM TRA CÓ KỸ THUẬT L KHÔNG ===
    const hasL = order.includes('L');
    console.log('🔍 [DECRYPT] Has L in order:', hasL);

    if (hasL) {
        const lTech = saoyut.techniques.find(t => t.name === 'L');
        if (lTech) {
            console.log('🔍 [DECRYPT] L metadata:', lTech.metadata);
            const dataId = lTech.metadata.data_id;
            const numSegments = lTech.metadata.num_segments;
            console.log(`🔍 [DECRYPT] Need ${numSegments} segments with data_id: ${dataId}`);
        } else {
            console.log('🔍 [DECRYPT] L in order but not in SAOYUT - will be skipped');
            showStatus('ℹ️ Kỹ thuật L trong order nhưng không có trong SAOYUT, sẽ bỏ qua', 'info');
        }
    }

    // === HIỂN THỊ TRẠNG THÁI ===
    showStatus('⏳ Đang giải mã với ' + order.length + ' kỹ thuật...', 'info');

    try {
        const requestBody = {
            encrypted_b64: input,
            seed: seed,  // Gửi seed thuần, server sẽ tự thử các biến thể
            saoyut: saoyut,
            order: order,
            session_id: browserSessionId
        };

        console.log('🔍 [DECRYPT] Request body:', {
            encrypted_b64_length: requestBody.encrypted_b64.length,
        ...
        const result = await response.json();
        console.log('🔍 [DECRYPT] Response result:', result);

        if (!result.success) {
            showStatus('❌ ' + result.error, 'error');
            if (result.hint) {
                setTimeout(() => showStatus('💡 ' + result.hint, 'warning'), 100);
            }
            return;
        }
        ...
        if (originalText) {
            originalText.innerHTML = `<span style="color:#888;font-size:11px;">📦 Base64 (input)</span><br>${input.substring(0, 60)}${input.length > 60 ? '...' : ''}`;
            originalText.style.color = '#f39c12';
        }
        if (decryptedText) {
            decryptedText.innerHTML = `<span style="color:#888;font-size:11px;">📝 Plaintext (output)</span><br>${result.decrypted_text}`;
            decryptedText.style.color = '#00ff88';
        }
...
function clearAll() {
    ...
    document.getElementById('keyPhienDisplay').textContent = 'Chưa có';
    clearSAOYUT();
    ...
    const hint = document.getElementById('seedDisplayHint');
    if (hint) hint.textContent = '💡 Seed này đã gắn với phiên trình duyệt hiện tại';
}
